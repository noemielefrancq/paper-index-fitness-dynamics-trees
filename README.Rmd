...
*Below is a brief example of what can be archived on SARS-CoV-2, using the codes in the folder ```2_Codes```.*
...
Loading SARS-CoV-2 tree, in which all the tip name include: collection time, location and Pango lineage 
```{r, echo=T, eval=FALSE}
...
Optimize the number of groups: set the minimum number of sequences per group to 20, with a minimum frequency of 10%.
```{r, eval = T, eval=FALSE}
...
Plot SARS-CoV-2 trees colored with each set of groups next to each other:
```{r, eval = T, eval=FALSE}
